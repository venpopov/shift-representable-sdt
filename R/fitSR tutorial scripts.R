...
# y1 is a list of 3 components; participants list a list of N matrices

y1 <- readexp(1, "data/dunn2025/") # acquire response data from Experiment 1
y1$participants[[1]] # show response data for participant 1
y1$total # show total response data (summed over participants)
y1$mean # show mean response data (averaged over participants)

out <- fitSR(y1$total) # analyse total response data with default 20 steps
str(out, 1) # display summary of results
out <- fitSR(y1$total, nstep = 40, input = out) # continue analysis a further 40 steps (60 in total)
str(out, 1) # display summary of results
